...
def build_command(terrainbuilder_path, tile_x, tile_y, zoom, output_dir, args):
    output_dir = os.path.join(output_dir, str(zoom), str(tile_x), f"{tile_y}.glb")
...
def process_tile(terrainbuilder_path, tile_x, tile_y, zoom, output_dir, args, build_intermediate):
    if build_intermediate or zoom == args.target_zoom:
        print(f"({zoom}, {tile_x}, {tile_y})")
        build_command(terrainbuilder_path, tile_x, tile_y, zoom, output_dir, [str(arg) for arg in args.command_args])
        print();

    if zoom < args.target_zoom:
        children = get_children(tile_x, tile_y)
        for child in children:
            process_tile(terrainbuilder_path, child[0], child[1], zoom + 1, output_dir, args, build_intermediate)
...
def main():
    ...
    process_tile(terrainbuilder_path, tile_x, tile_y, root_zoom, args.output_dir, args, args.build_intermediate)
