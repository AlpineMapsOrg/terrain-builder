    ...
    const glm::dvec2& l1)
{
    ...
    const double c_x = ((x_1 * y_2 - y_1 * x_2) * (x_3 - x_4) - (x_1 - x_2) * (x_3 * y_4 - y_3 * x_4)) / denom;
...
BBox2D::BBox2D(glm::vec2 a, glm::vec2 b)
    : BBox2D(glm::dvec2(a.x, a.y), glm::dvec2(b.x, b.y))
{
}

BBox2D::BBox2D(glm::vec3 a, glm::vec3 b)
    : BBox2D(glm::dvec2(a.x, a.y), glm::dvec2(b.x, b.y))
{
}

BBox2D::BBox2D(glm::dvec3 a, glm::dvec3 b)
    : BBox2D(xy(a), xy(b))
{
}

BBox2D::BBox2D(const Triangle& t)
    : BBox2D(xy(t[0]), xy(t[1]))
{
    add(xy(t[2]));
}
...
bool BBox2D::intersects(const BBox2D& other, const double epsilon) const
{
    ...
    // is this above other?
    if (min.y - epsilon > other.max.y + epsilon)
        return false;

    // is this below other?
    if (max.y + epsilon < other.min.y - epsilon)
        return false;

    // is this left of other?
    if (max.x + epsilon < other.min.x - epsilon)
        return false;

    // is this right of other?
    if (min.x - epsilon > other.max.x + epsilon)
        return false;
...
{
    return (min.x - epsilon) <= point.x && (min.y - epsilon) <= point.y && (max.x + epsilon) >= point.x && (max.y + epsilon) >= point.y;
}
...
{
    return epseq(point.x, min.x, epsilon) || epseq(point.x, max.x, epsilon) || epseq(point.y, min.y, epsilon) || epseq(point.y, max.y, epsilon);
}
...
BBox3D::BBox3D(const Triangle& t)
    : BBox3D(t[0], t[1])
{
    add(t[2]);
}
...
{
    return (min.x - epsilon) <= point.x && (min.y - epsilon) <= point.y && (min.z - epsilon) <= point.z &&

        (max.x + epsilon) >= point.x && (max.y + epsilon) >= point.y && (max.z + epsilon) >= point.z;
}
...
lp[0] +              |
    ...
    */
    ...
    if (num_left_points == 0) {
        ...
        const glm::dvec3 s0 = intersect_25D_linesegment_by_line(other_points[0], left_points[0], l_org, l_dir);
        const glm::dvec3 s1 = intersect_25D_linesegment_by_line(other_points[0], left_points[1], l_org, l_dir);
