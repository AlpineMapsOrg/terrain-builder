...
TEST_CASE("progress indicator") {
  ...
  SECTION("parallel processing interface") {
    ...
    public:
      MockTileWriter() : ParallelTileWriterInterface(Tiler::Border::No, "empty") {}
      void write(const std::string&, const Tile&, const HeightData&) const override { std::this_thread::sleep_for(1ms); }
    };

    auto generator = ParallelTileGenerator::make(ATB_TEST_DATA_DIR "/austria/at_mgi.tif", ctb::Grid::Srs::SphericalMercator, Tiler::Scheme::Tms, std::make_unique<MockTileWriter>(), "./unittest_tiles/");
