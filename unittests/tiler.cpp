...
TEST_CASE("tiler bounds") {
  ...
  SECTION("mercator / quantised mesh level 1 and 2 (test with cape horn, on southern and western hemisphere)") {
    const auto grid = ctb::GlobalMercator();
    auto dataset = Dataset(ATB_TEST_DATA_DIR "/capehorn/small.tif");
    const auto tiler = Tiler(grid, dataset.bounds(grid.getSRS()), Tiler::Border::No);

    {
      ...
      REQUIRE(t.srsBounds.getMinX() == Approx(grid.getExtent().getMinX()));
      REQUIRE(t.srsBounds.getMinY() == Approx(grid.getExtent().getMinY()));
      REQUIRE(t.srsBounds.getMaxX() == Approx(0));
      REQUIRE(t.srsBounds.getMaxY() == Approx(0));
    }
    {
      const auto l2_tiles = tiler.generateTiles(2);
      REQUIRE(l2_tiles.size() == 1);
      ...
      REQUIRE(t.srsBounds.getMinX() == Approx(grid.getExtent().getMinX() + grid.getExtent().getWidth() / 4.0));
      REQUIRE(t.srsBounds.getMinY() == Approx(grid.getExtent().getMinY() + grid.getExtent().getHeight() / 4.0));
      REQUIRE(t.srsBounds.getMaxX() == Approx(0));
      REQUIRE(t.srsBounds.getMaxY() == Approx(0));
    }
  }

  SECTION("geodetic / cesium terrain raster level 1 and 2 (test with cape horn, on southern and western hemisphere)") {
    const auto grid = ctb::GlobalGeodetic(64);
    auto dataset = Dataset(ATB_TEST_DATA_DIR "/capehorn/small.tif");
    const auto tiler = Tiler(grid, dataset.bounds(grid.getSRS()), Tiler::Border::Yes);

    {
      ...
      const auto t = l1_tiles.front();
      REQUIRE(t.zoom == 1);
      REQUIRE(t.point == ctb::TilePoint(1, 0));
      REQUIRE(t.gridSize == 64);
      REQUIRE(t.tileSize == 65);

      constexpr auto tileSizeX = 360 / 4.0;
      constexpr auto tileSizeY = 180 / 2.0;
      REQUIRE(t.srsBounds.getMinX() == Approx(-tileSizeX));
      REQUIRE(t.srsBounds.getMinY() == Approx(-tileSizeY));
      REQUIRE(t.srsBounds.getMaxX() == Approx(tileSizeX / 64.0));
      REQUIRE(t.srsBounds.getMaxY() == Approx(tileSizeX / 64.0));
    }
    {
      const auto l2_tiles = tiler.generateTiles(2);
      REQUIRE(l2_tiles.size() == 1);
      const auto t = l2_tiles.front();
      REQUIRE(t.zoom == 2);
      REQUIRE(t.point == ctb::TilePoint(2, 0));
      REQUIRE(t.gridSize == 64);
      REQUIRE(t.tileSize == 65);

      constexpr auto tileSizeX = 360 / 8.0;
      constexpr auto tileSizeY = 180 / 4.0;
      REQUIRE(t.srsBounds.getMinX() == Approx(-2 * tileSizeX));
      REQUIRE(t.srsBounds.getMinY() == Approx(-90));
      REQUIRE(t.srsBounds.getMaxX() == Approx(-tileSizeX + tileSizeX / 64.0));
      REQUIRE(t.srsBounds.getMaxY() == Approx(-90 + tileSizeY + tileSizeX / 64.0));
    }
  }
}
