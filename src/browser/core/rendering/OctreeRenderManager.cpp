...
namespace octree
{
    ...
    // void OctreeRenderManager::populate_gpu_nodes(glm::dvec3 cam_pos)
    // {
    //     LOG_DEBUG("Starting GPU Octree Population");
    ...
    //             std::optional<GPUOctreeNode> node = GPUOctreeNode::load(current, m_storage, m_space);

    //             if (!node.has_value())
    //             {
    //                 LOG_DEBUG("Node {} couldn't be loaded!", current);
    //             }

    //             m_cached_octree_nodes.push_back(node.value());
    //             m_cached_octree_node_ids.push_back(current);
    //             return;
    //         }
    //     }
    // }
    ...
    // OctreeRenderIntent OctreeRenderManager::generate_visible_octree_nodes(glm::dvec3 cam_pos)
    // {
    //     std::vector<float> instance_active;
    //     std::vector<glm::mat4> instance_models;

    //     for (Id id : m_cached_octree_node_ids)
    //     {
    //         Bounds bounds = m_space.get_node_bounds(id);
    ...
    // void OctreeRenderManager::render_gpu_nodes(Uniform<glm::mat4> U_model_mesh, glm::dvec3 cam_pos)
    // {
    //     for (GPUOctreeNode &node : m_cached_octree_nodes)
    //     {
    //         // Id id = entry.first;
    //         // GPUOctreeNode &node = entry.second;

    //         node.recenter(cam_pos);

    //         U_model_mesh.set(node.model_matrix());
    //         node.render();
    //     }
    // }
