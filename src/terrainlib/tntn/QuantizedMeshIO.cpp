...
QuantizedMeshVertexData detail::quantised_mesh_vertex_data(const Mesh& m, const BBox3D& bbox, const OGRSpatialReference& srs, bool mesh_is_rescaled)
{
    ...
    for (auto it = triangles.begin; it != triangles.end; ++it) {
        for (int n = 0; n < 3; ++n) {
            ...
            if (vertices_order_it != vertices_order.end())
                continue;
    ...
    bool mesh_is_rescaled)
{
    ...
    TNTN_ASSERT(
        bio.write_pos() == sizeof(QuantizedMeshHeader)); // might not be true for some platforms, mostly for debugging
...
static BBox3D bbox_from_header(const QuantizedMeshHeader& qmheader)
{
    ...
    const glm::dvec2 bottom_right = qmheader.bounding_sphere_center + qmheader.BoundingSphereRadius;
...
std::unique_ptr<Mesh> load_mesh_from_qm(const std::shared_ptr<FileLike>& f)
{
    ...
    std::vector<Vertex> vertices = decode_qm_vertices(header_bbox, vertex_data_u, vertex_data_v, vertex_data_height);
