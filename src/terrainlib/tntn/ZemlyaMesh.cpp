    ...
    void ZemlyaMesh::greedy_insert(double max_error)
    {
            ...
            for (int y = 0; y < h; y += pow(2, step)) {
                for (int x = 0; x < w; x += pow(2, step)) {
                    if (step == 1) {
                        ...
                        if (y + co < h && x + co < w) {
                            m_sample.value(y + co, x + co) = average_of(v1, v2, v3, v4, no_data_value);
                        }
                    }
                }
            }
        ...
        for (int level = 1; level <= m_max_level; level++) {
                ...
                if (step >= 3) {
                    ...
                    for (int y = 0; y < h; y++) {
                        for (int x = 0; x < w; x++) {
                            ...
                            const double v1 = y - d < h && x - d < w ? m_sample.value(y - d, x - d) : NAN;
                            const double v2 = y - d < h && x + d < w ? m_sample.value(y - d, x + d) : NAN;
                            const double v3 = y + d < h && x - d < w ? m_sample.value(y + d, x - d) : NAN;
                            const double v4 = y + d < h && x + d < w ? m_sample.value(y + d, x + d) : NAN;
            ...
            while (!m_candidates.empty()) {
                ...
                if (m_token.value(candidate.y, candidate.x) != candidate.token)
                    continue;
        ...
        const double no_data_value)
    {
        ...
        for (int x = startx; x <= endx; x++) {
            if (!m_used.value(y, x)) {
                ...
                const double z = m_current_level == m_max_level ? m_raster->value(y, x) : m_insert.value(y, x);
                if (!terra::is_no_data(z, no_data_value)) {
